...
module.exports = Addon.extend({
  ...
  isParentAnInRepoAddon: function() {
    console.log('checking if the parent of my parent is an in-repo addon');
    ...
    if (grandparent) {
      ...
      if (pkg) {
        console.log('Found grandparent pkg', Object.keys(pkg));
          ...
          if (addonConfig.paths) {
            addonConfig.paths.forEach(function(relativePath) {
              ...
              if (addonPath === parentRoot) {
                console.log('My parent is an in-repo addon!');
                result = true;
              }
            });
          }
