...
module.exports = Addon.extend({
  ...
  _treeForTranslation: function() {
    if (this._I18N_TREES['_treeForTranslation']) {
      return this._I18N_TREES['_treeForTranslation'];
    }
    ...
    // console.log(!!this.project, getAddonPathToParent(this), '\n\n');

    this._I18N_TREES['_treeForTranslation'] = mergeTrees(trees);

    return this._I18N_TREES['_treeForTranslation'];
  },

  _treeForTranslatedProperties: function() {
    if (this._I18N_TREES['_treeForTranslatedProperties']) {
      return this._I18N_TREES['_treeForTranslatedProperties'];
    }
    ...
    if (existsSync(i18nPath)) {
      ...
      this._I18N_TREES['_treeForTranslatedProperties'] = new Funnel(i18nPath, {
        getDestinationPath: function(relativePath) {
  ...
  _collectTranslationTrees: function() {
    if (this._I18N_TREES['_collectTranslationTrees']) {
      return this._I18N_TREES['_collectTranslationTrees'];
    }

    var trees = collectAddons(this.parent, result(this, 'name'));

    this._I18N_TREES['_collectTranslationTrees'] = trees.map(function(addon) {
